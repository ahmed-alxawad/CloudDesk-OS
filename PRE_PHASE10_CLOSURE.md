...
| 4 | Video real-browser acceptance (playback/seek/speed/fullscreen/subtitles/track-switch) | **PASS (Phase 4)** | `services/clouddeskd/tests/video_playwright.rs` (7/7: real compiled frontend, Files→double-click→Video open, real probe/plan/job flow, direct/remux/transcode paths, real seek/pause/resume/mute/volume/ended, real 206 range responses, authorization + cross-user revocation, refresh/reopen leak-free, no ffmpeg/console leaks); driver `services/clouddeskd/tests/browser/video_flow.mjs`. Fixed 2 real product defects found live: (1) `VideoApp.svelte`/`video.ts` always requested a `remux` job for a `transcode`-plan file, permanently unplayable — now uses `conversionOperationFor(plan)`; (2) `crates/media/src/exec.rs::remux()` always muxed to `.mp4`, which has no codec tag for VP8/VP9 — a real WebM/VP8 remux source always failed `ffmpeg_failed` — now probes the source codec and outputs `.webm` when needed. Playwright's Chromium build has no H.264/AAC decoder (confirmed via `MediaSource.isTypeSupported`, a real licensing-driven environment limit, not a CloudDesk defect); direct-path fixture uses VP9/Opus instead, and the transcode path's real H.264/AAC output is verified independently via Rust-side `ffprobe` rather than in-browser decode. `cloudesk-privd` requires real root (unavailable in this sandbox); Files-listing evidence uses the real `clouddesk_privilege` wire protocol dispatched directly to the real, unmodified `cloudesk-sessiond` worker binary, substituting only the root-owned outer relay. | Yes | Closed | None |
| 5 | Music application backend/router/data path | **PASS (reconciled, Phase 5)** | `V1_TRUE_CLOSURE.md` #3 CLOSED; `crates/library` (roots/tracks/artists/albums/playlists/favorites/recent/queue, all owner-scoped at the SQL layer) + `music_api.rs`/`music_authorization.rs` (real ffmpeg fixtures, cross-user attack matrix) + `music_persistence.rs` (restart survival, no duplicate reindex) + `scan_live.rs` (format-agnostic indexing across MP3/FLAC/WAV/OGG-Vorbis/AAC-M4A, Unicode tags, untagged fallback); frontend `music.ts`/`MusicApp.svelte`. Two real defects found and fixed this pass: (1) `MusicApp.svelte` always requested a `remux` job regardless of probed plan (identical defect class to Phase 4 Video, independently present in Music) -- now uses the shared `conversionOperationFor(plan)` (moved to `media.ts`, re-exported by both `video.ts` and `music.ts`); (2) `crates/media/src/probe.rs` only ever read `format.tags`, never a stream's tags -- Ogg/Vorbis (and other Vorbis-comment containers) report title/artist/album on the audio stream, not the format, so every real OGG track's metadata was silently dropped; probe now falls back to the first audio stream's tags when format-level tags are empty. | Yes | Closed | None |
| 5 | Music real-browser acceptance (library/play/queue/playlist/favorite/search) | **PASS (Phase 5)** | `services/clouddeskd/tests/music_playwright.rs` (3/3: `task_full_product_journey` -- Files double-click opens the exact clicked track, real play/pause/resume/seek, add-folder+scan via the real product prompt UI, favorite/queue/playlist/search/artist-grouping/recently-played all proven against real indexed tracks with real 206 range responses; `task_corrupt_fixture_shows_safe_failure`; `task_close_during_playback_leaves_no_process_leak`); driver `tests/browser/music_flow.mjs`. Reuses Phase 4's privilege-relay substitution (real `clouddesk_privilege` wire protocol -> real `cloudesk-sessiond` binary, root-owned outer relay only substituted, same disclosed rationale). | Yes | Closed | None |
| 6 | Optional-runtime orchestrator (`crates/orchestrator`) | PASS (COMPLETE) | `PHASE6_RUNTIME_EVIDENCE.md`: 38/40 PASS, 1 NOT EXECUTED (persistent-profile retention — no persistent-kind adapter existed at that time; Browser now provides one, see Phase 9 row below), 3 `BLOCKED BY ENVIRONMENT` (cgroup CPU/memory/PIDs enforcement, re-confirmed unchanged this pass, not re-investigated per this pass's own scope). Settings browser acceptance is now **PASS (Phase 6)** — see next row. | Yes | — | None |
| 6 | Settings browser acceptance (enable/disable runtime cards) | **PASS (Phase 6)** | `services/clouddeskd/tests/settings_playwright.rs` (2/2: `task_admin_runtime_lifecycle_through_settings` -- Administrator sees all three real cards, then for each of Browser/Code/Office through the real compiled UI: enable -> real launch (Brave remote-page canvas / Code iframe / Files -> Open with Office real ODT) -> disable through Settings -> 0 resident Docker containers (polled) -> re-enable -> real launch works again; `task_non_admin_has_no_runtime_controls` -- an ordinary User sees the cards but never an enable/disable button); driver `tests/browser/settings_flow.mjs`. Reuses Phase 4/5's privilege-relay substitution for Files (same disclosed rationale). Three real defects found and fixed this pass: (1) `App.svelte`'s launcher opened the grid with stale runtime-availability flags (a real race against its own background reconciliation fetch) -- a just-enabled app could show "unavailable under current policy" on the very next click; now awaits reconciliation before opening. (2) `BrowserApp.svelte` always created a brand-new runtime instance on (re)connect, and a `Stopped` row deliberately still counts against the per-user instance limit (default 1) -- any Browser session that ever disconnected left that user permanently unable to open Browser again with a 429. (3) The server-side instance-reuse pattern already shipped for Code (`existing_code_instance`) was generalized to also cover Browser (`existing_instance_for_reuse`, excluding `Failed` rows to preserve the existing, already-tested real-restart-reconciliation guarantee). | Yes | Closed | None |
...
**Not retained as environment blockers** (explicitly re-evaluated per Part Z/AB, since the Playwright/Chromium harness now exists from Phase 8's `office_browser.rs`): Settings and Code browser acceptance are now `NOT EXECUTED`, not `BLOCKED BY ENVIRONMENT` — the tooling exists, the acceptance runs simply have not been performed yet. Video browser acceptance is now **PASS (Phase 4)** and Music browser acceptance is now **PASS (Phase 5)** — see rows above.
...
- Mandatory `NOT EXECUTED`: **~2** (Settings/Code browser acceptance ×2; Video browser acceptance is **PASS** (Phase 4) and Music browser acceptance is now **PASS**, closed this pass (Phase 5); Phase 3's timeout/quota live-fire is **PASS**, closed a prior pass — see rows for the authoritative list)
...
also COMPLETE** — see the section immediately above. **Phase 4 (Video
compiled-browser acceptance), Phase 5 (Music reconciliation +
compiled-browser acceptance), and Phase 6 (Settings runtime-card
compiled-browser acceptance) are now also COMPLETE** — see their rows
above. What still blocks readiness: Code product/browser acceptance
(Phase 7), still `NOT EXECUTED`.

**Next exact action**: pre-Phase-10 product acceptance closure --
reconcile and close Phase 7 (Code product/browser acceptance). Not
Phase 10.
