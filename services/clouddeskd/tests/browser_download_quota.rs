...
/// Task 4: a reduced, test-only quota is exercised through the exact
/// same live cancellation code path production uses.
#[tokio::test]
async fn task_4_download_quota_enforced() {
    ...
    let fixture_port = spawn_download_fixture("over_quota.bin").await;
    ...
    let user_cookie = create_user(&base, &admin_cookie, "dlquotauser", "user").await;
    ...
    // REAL_PAYLOAD is well over the 16-byte per-download test quota.
    let outcome = recv_json_matching(
        &mut rx,
        |v| v["type"] == "download_completed" || v["type"] == "download_failed",
        std::time::Duration::from_secs(15),
    )
    .await;
    assert_eq!(
        outcome.map(|v| v["type"].clone()),
        Some(json!("download_failed")),
        "a download exceeding the per-download quota must be cancelled, not completed"
    );
